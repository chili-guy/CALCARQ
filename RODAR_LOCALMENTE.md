...
### Passo 3: Verificar se existe arquivo .env na raiz
```bash
ls -la .env
```
...
3. Você deve ver a página inicial do Calcarq!
...
Verifique se o arquivo `server/.env` existe e tem a chave configurada.
...
Você pode usar o script `start-dev.sh` que já existe:
...
Mas ainda precisará do Stripe CLI rodando em um terminal separado.
